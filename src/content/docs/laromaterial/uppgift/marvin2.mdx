...
Du har jobbat igenom allt material i "[kmom03](/kmom/kmom03)".
...
1. Strukturera koden:
    ...
    - `main.py` ska innehålla koden för att starta ditt program och while-loopen som utgör menyn. 
...
1. Koden för dina menyval ska nu flyttas till olika funktioner i `marvin1.py`. Med "koden för menyval" menas koden som utgör funktionaliteten för ett menyval, t.ex.
    ...
    * Menyval 3 - `compare_numbers`
    * Menyval 4 - `validate_ssn`
    ...
    **Alla** `input()` och `print()` som används i menyvalen skall ligga **i funktionen för menyvalet** och inte i main funktionen.  
    ...
    - Tags: `struct`. Det går att använda `struct` som tag i `uv run task test -- kmom03 --tags struct` för att köra testerna som kollar att man har rätt filer på rätt plats.
...
1. Menyval **5** - `create_ssn`: Skapa de fyra sista siffrorna till ett födelsedatum. Skapa ett nytt menyval där Marvin ber om en sträng som innehåller ett födelse datum, t.ex. `"910813"`. Din funktion ska skapa de fyra sista siffrorna till det, `"910813-NNNN"`. För att skapa dem, använda [random modulen](https://docs.python.org/3.8/library/random.html) och slumpa fram de tre första siffrorna. Använd sen Luhnalgoritmen för att räkna ut den fjärde. Den funkar nästa på samma sätt som i menyval 4. 
    ...
    *   2  1 2 1 2 1  2 1  2
    ...
    Som du läst finns det funktionalitet som är samma mellan menyval 4 och 5. Därför ska du skapa en ny funktion som används i koden för menyval 4 och 5. Skapa `create_ssn` som tar emot födelsedatumet och skapar tre siffror. Skapa sen `calculate_luhna_sum` som tar emot en sträng med födelsedatumet och de tre siffrorna, utan `-`, `"811218987"`. Funktionen ska räkna ut summan och returnera den, `44`. I `create_ssn` använd summan och räkna ut sista siffran och returnera hela personnumret, med `-`, `811218-9876`.

    Uppdatera sen koden för menyval 4 så att du använder `calculate_luhna_sum` i den koden också, för att räkna ut summan som ska kollas om den är delbar med 10. Kolla om talet är delbart med 10 i menyval 4 funktionen, inte i `calculate_luhna_sum`. Genom att återanvända funktioner på flera ställen kan vi få mer [DRY kod](https://en.wikipedia.org/wiki/Don%27t_repeat_yourself).
...
1. Menyval **6** - `roll_dice`: Kasta tärningar. Marvin ska be dig skriva in ett tärningsuttryck. Tärningsuttrycket är på formatet `NdM`, till exempel `2d6`, där `N` är antalet tärningar (1–9) och `M` är antalet sidor på varje tärning. Funktionen `roll_dice` simulerar att kasta `N` stycken `M`-sidiga tärningar och returnerar summan som ett heltal. Resultatet skrivs ut i formatet `"NdM: <summa>"` i `main.py`.

    Tärningsuttrycket kan också ha ett tillägg i formatet `NdM+K` eller `NdM-K`, där `K` är ett heltal som adderas respektive subtraheras från summan. Resultatet skrivs då ut i formatet `"NdM+K: <summa>"` respektive `"NdM-K: <summa>"`.


    ```python

    input: "2d6"    output: "2d6: 7"      (summan av två sexsidiga tärningar, slumpas 2-12)
    input: "1d20"   output: "1d20: 15"    (en tjugosidis tärning, slumpas 1-20)
    input: "3d6+5"  output: "3d6+5: 16"   (tre sexsidiga tärningar plus 5, slumpas 8-23)
    input: "2d8-3"  output: "2d8-3: 6"    (två åttasidiga tärningar minus 3, slumpas -1 till 13)
    ```
...
- Använd `uv run task test -- kmom03 --tags <menyval>` för att bara köra testerna för menyvalet du jobbar med. Du kan använda `uv run task test -- kmom03 -s` för att se alla taggar som finns och vilka tester de kör.
