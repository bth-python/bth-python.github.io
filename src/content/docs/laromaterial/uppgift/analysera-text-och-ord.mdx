...
Du har en bra förståelse för `dictionary` och har jobbat igenom [Dictionaries och tupler i Python](/laromaterial/övning/dictionaries-och-tupler-i-python) och [Att läsa filer till en lista i Python](/laromaterial/övning/att-lasa-filer-i-python).
...
1. Ni får inte använda externa moduler för att göra analyzern. T.ex. får ni inte använda `collections` modulen. Ni ska skriva koden själva. Däremot får ni använda t.ex. `itemgetter` för att sortera er data.
...
PS. Ni måste identifiera någon kod som kan återanvändas i både `word_frequency` och `letter_frequency` och skriva en funktion för det i `analyzer.py`.
    ...
    - Tags: `freq` (gemensam för båda), `word_frequency`, `letter_frequency`. 
...
- Kör `uv run ruff check` och `uv run task test -- kmom06` för att kontrollera din lösning