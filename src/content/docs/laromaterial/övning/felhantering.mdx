---
title: "Felhantering i Python"
description: Lär dig hantera felhantering med try-except.
sidebar:
  order: 0212
---
...
<Figure
    src={var1Image}
    caption="Visar ett fel kastat i programmet."
    class="right"
/>
...
Fel kan alltid uppstå i vår kod, speciellt när vi jobbar med input från en användare. Vi kan inte hindra användaren från att skicka in data som skiljer sig från vad vi vill att de ska skicka in. Detta måste vi hantera, annars kommer vårt program att krascha vilket vi aldrig vill att det ska ske.
...
Felet visar oss vilken fil det handlar om (i detta fallet interpretatorn). Den lilla pilen pekar på den tidigaste gången felet upptäcktes på raden och vanligen är den felaktiga koden i delen före pilen. Här handlar det om att if-satsen inte startar med `:` så här `if 100 < 5: print("Nu då...")`.
...
Att använda try-except för att göra något när ett exceptions uppstår kallas att fånga/catch exceptions. När koden skapar ett exception uppstår exception kallar vi antingen lyfta/raise eller kasta/throw exception.
...
Det går att fånga alla typer av exceptions utan att behöva specificera vilka man vill fånga. Det är vanligt att man ser den lösningen när man letar efter hjälp på nätet. Men det är en dålig lösning som inte ska användas. I kursen, när ni ska lösa uppgifter är det inte giltigt med denna lösningen, så vida det inte står i uppgiften. Ni ska specificera vilka fel ni vill fånga.
...
Vi har haft fokus på felhantering i Python. Ett exception är ett fel som uppstår under körning av ett program. När ett fel inträffar, "kastas" ett exception, vilket kan hanteras för att undvika att programmet kraschar. Vi använder **try-except** med kod som kan orsaka fel i try-blocket och så tar vi hand om felet eller felen i except-blocket.
