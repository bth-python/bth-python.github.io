---
title: "Introduktion till variabler och datatyper"
description: Lär dig om värden och datatyper i Python. Vi går igenom heltal, decimaltal, strängar och booleska värden. Vi skriver ett program som tar emot inmatning från användaren.
sidebar:
  order: 0110
---
...
import var1Image from '@assets/img/ovning/variable-memory1.png';
import var2Image from '@assets/img/ovning/variable-memory2.png';
import var3Image from '@assets/img/ovning/variable-memory3.png';
import var4Image from '@assets/img/ovning/variable-memory4.png';
import var5Image from '@assets/img/ovning/variable-memory5.png';
import var6Image from '@assets/img/ovning/variable-memory6.png';
import var7Image from '@assets/img/ovning/variable-memory7.png';
import var8Image from '@assets/img/ovning/variable-memory8.png';
...
Du kan hitta koden för detta exempel på [Github](https://github.com/dbwebb-se/python/tree/master/example/greeting).
...
Inom programmering används värden, t.ex. heltalet 4 (Integer) eller en sträng "Hello world" (String). Värden är av olika typer, vilket ger dem olika egenskaper i koden. Nedan beskrivs de fyra av de vanliga typerna som finns i programmeringsspråk; heltal, decimaltal, sträng och boolean.
...
Följande kod är exekverad i Python-interpretatorn, du startar interpretatorn genom att skriva `uv run python` i din terminal. När du läser nedanstående kodexempel rekommenderar vi att du själv har en interpretator öppen och kodar med. För att lämna Python-interpretatorn trycker du `ctrl + d` eller skriver `exit()`.

```python
>>> 4 + 4 # >>> betyder att vi skriver in detta i interpretatorn. Raden under är utskriften från interpretatorn.
8
>>> 0 * 8
0
>>> 5 // 2
...
Notera det tredje och fjärde beräkningarna i exemplet ovan. `//` är heltalsdivision och räknar ut hur många hela tal som får plats vid division. `%` är modulusoperatorn och räknar ut resten vid heltalsdivision. Vår beräkning av `5 % 2` får resultatet 1, dvs om vi gör en heltalsdivision med 5 och 2 är resten 1. Detta kan till exempel användas om vi vill avgöra siffran är jämn eller udda. Får vi resultatet 1 vet vi att siffran är udda och är resultatet av uttrycket 0, dvs ingen rest i heltalsdivisionen, är talet ett jämnt tal.


<YouTube
    id="tDl04edpj5w"
    title="Hur Python exekverar operationer och matteregler."
/>
...
**Notera** att rent matematiskt borde 3.14 + 3 bli 6.14 och 0.1 + 0.2 bli 0.3. Datorn kan inte lagra alla decimaltal exakt, så den lagrar den närmaste möjliga approximationen. Till exempel kan inte talen 0.1 och 3.14 representeras exakt. Därför uppstår ibland små avrundningsfel som syns när talen skrivs ut.
...
Textvärden heter [_sträng_](https://docs.python.org/3/library/stdtypes.html#text-sequence-type-str) på svenska och _string_ på engelska (förkortas med __str__). En sträng är en sekvens av tecken omslutna av enkla (') eller dubbla (") citattecken. Vi vet att `"Hello world"` är en sträng för att den är omsluten av citattecken. En sträng kan inte innehålla samma typ av citattecken som den är omsluten av och en sträng avslutas så fort ett likadant citattecken påträffas i sekvensen.
...
Om vi har två strängar som vi vill lägga ihop till en lång kan detta göras med ett "+" mellan två strängar. Detta kallas att konkatenera strängar, den engelska översättningen är 'concatenate'.
...
:::tip
Vi använder `$` för att visa att det är ett terminalkommando. Skriv **inte** med `$` när du skriver kommandot i terminalen.
:::
```bash
# Gå till ditt kursrepo python
$ cd src/kmom01/practice
$ touch greeting.py
$ code .
```
...
<Figure
    src={var1Image}
    caption="an_integer refererar till värdet 23 i minnet."
    class="right"
/>
...
<Figure
    src={var2Image}
    caption="Flera variabler refererar till värden i minnet."
    class="right"
/>
...
<YouTube
    id="6Ru8gJOV4bo"
    title="Hur variabler kopplas till värden i minnet."
/>



## Namngivning och Keywords
...
Python är ett så kallat sekventiellt programmeringsspråk. Det betyder att när vi kör programmet kör vi det ovanifrån och ner, en rad i taget. Vi vet alltid att vi är klara med raden ovan när vi börjar på nästa rad.
...
Kom ihåg att koden exekveras uppifrån och ner och en rad åt gången. När vi testar programmet kommer programmet börja med att vänta på att användaren ska skriva något i terminalen och klicka "enter". Sen exekveras nästa rad som skriver ut vår hälsning.
...
<Figure
    src={var4Image}
    caption="variabeln 'name' har värdet av det vi matade in till input()."
    class="right"
/>
...
<Figure
    src={var5Image}
    caption="Variablerna 'name' och 'greeting' har värden i minnet."
    class="right"
/>
...
<YouTube
    id="zH8j_GZkkT4"
    title="Hur input() och string konkatenering exkeveras i Python."
/>
...
```python
year = 2025 # Hårdkodat värde för vilket år det är

name = input("Skriv ett namn, klicka sen enter: ")
age = input("Skriv en ålder, klicka sen enter: ")

year_born = year - age # Födelseår räknas ut. (2025 - inmatat värde)
...
Var du beredd på felet? Jag skrev tidigare att allt `input()` returnerar är av typen sträng och vi har sett att vi inte kan blanda strängar och heltal i operationer. Därför får vi ett TypeError fel på rad 11 i `greeting.py` när vi försöker göra `2025 - "27"`.
...
Nu räknar vi ut `year_born` med `year` (2025) minus `age` (inmatat värde). Kan du se nästa fel?
...
<Figure
    src={var7Image}
    caption="Variabler refererar till värden i minnet."
    class="right"
/>

<YouTube
    id="BZRYlIlH0vw"
    title="Typkonvertering och korta ner rad i Python"
/>
