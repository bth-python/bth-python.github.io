## Kursen Python


**Introduktion till kursen**


Andreas Arnesson




## Agenda
<hr style="color:green">

* Kursformalia
* Innehåll
* Labbmiljö




## Hitta kursmaterialet
<hr style="color:green">

* Gå in på [canvas.bth.se](https://canvas.bth.se) och klicka på länkar
* Gå till [bth-python.github.io/](https://bth-python.github.io/)





## Översikt av kursen
<hr style="color:green">

* [bth-python.github.io/intro](https://bth-python.github.io/intro)

Läs igenom artiklarna under "introduktion" så har du koll. Det är din innehållsförteckning av kursen.




## Kursnamn och kod
<hr style="color:green">

* Programmering och Problemlösning i Python
* DV1670
* 7.5hp
* Datavetenskap/Programvaruteknik




## Kursplan
<hr style="color:green">

* Kursplanen är ett formellt styrdokument till kursen som bla beskriver detaljer, förkunskaper, syfte, innehåll, mål och litteratur.

* Låt oss kika på en del information som går att läsa i kursplanen.

<p class="footnote">Kursplan: <a href="https://edu.bth.se/utbildning/utb_kursplaner.asp?KKurskod=DV1670">https://edu.bth.se/utbildning/utb_kursplaner.asp?KKurskod=DV1670</a></p>




## Förkunskaper

    Grundläggande behörighet




## Innehåll
<hr style="color:green">

* Problemlösning
* Grundläggande programmering
* Datastrukturer
* Standard bibliotek
* Felhantering och felsökning
* Filhantering
* Utveckling




## Mål kursplan
<hr style="color:green">

* Kunskap och förståelse


    "Påvisa grundläggande kunskaper i problemlösning med strukturerad programmering i Python genom att skriftligen beskriva och sammanfatta erfarenheter och observationer från övningar och uppgifter."

    "Påvisa grundläggande kunskaper i att lösa definierade problem via programmering i Python"




## Mål kursplan...
<hr style="color:green">

* Färdighet och förmåga

    "Utveckla en lösning i Python, utifrån en specifikation, samt dokumentera och skriftligt kunna presentera denna lösning."

    "Ha god praktisk förmåga att hantera de verktyg och utvecklingsmiljöer som används vid utveckling och felsökningar med Python."




## Mål kursplan...
<hr style="color:green">

* Värderingsförmåga och förhållningssätt

    "Självständigt och kritiskt kunna utvärdera och analyser sina lösningar."

    "Bedöma vilka konstruktioner som är lämpliga att använda för ett problem."




## Kursliteratur
<hr style="color:green">

* Bok: Python for Everybody: Exploring data using - Charles R Severance
* Material från institutionen.
* Resurser via nätet

<p class="footnote">Läsanvisningar finns i varje kmom.</p>




## Planering
<hr style="color:green">

* [https://bth-python.github.io/intro/studieplan/](https://bth-python.github.io/intro/studieplan/)
    * Rekommenderad studieplan
    * BTH Schema




## Läromiljö
<hr style="color:green">

* [Canvas](https://bth.instructure.com/)
* [https://bth-python.github.io/](https://bth-python.github.io/)
* [Kursrepo GitHub](https://github.com/orgs/bth-python/repositories)
* Discord
* Zoom
* [YouTube](https://www.youtube.com/@DbwebbSe)




## Intressenter
<hr style="color:green">

* Studenter
* Lärarteamet
* Programansvariga
* Institutionen för Datavetenskap
    * Prefekt
    * Studierektor
* Utbildningsföreningen BITS




## Examination
<hr style="color:green">

* Varje kmom lämnas in för kontroll (en eller flera uppgifter)
    * U, Ux, PG, G
* Individuell examination med projektet och muntligt förhör
    * A-F, Fx




## Ladok
<hr style="color:green">

| Moment | Beskrivning | Poäng | Betyg |
|--------|-------------|-------|-------|
|Kmom01-03|Inlämningsuppgift 1|2,5hp|GU|
|Kmom04-06|Inlämningsuppgift 2|2,5hp|GU|
|Kmom10|Individuellt projekt|2,5hp|GU|


<p class="footnote">Kursens slutbetyg är enligt FA när samtliga moment är avklarade.</p>




## Kursvärdering & kursutveckling
<hr style="color:green">

- BTH kursvärdering
- Samla feedback på GitHub issue
- Feedback i redovisningstexter
- Avslutande kommentar om kursen med betyg på kursen




## Dialog, samarbete & fusk
<hr style="color:green">

- Diskutera med studiekollegor kring problem och lösningar på de övningar och uppgifter som finns
- Tveka inte att starta egna studiegrupper där ni kan stödja varandra.
- **Kopiera inte kod av varandra**
- [Fusk, AI och medvetet vilseledande](https://bth-python.github.io/studieguide/fusk/)
<p class="footnote">Allt misstänkt fusk anmäls.</p>





## Kursmoment
<hr style="color:green">

* 01: Kom igång med Python
* 02: Loopar
* 03: Funktioner och moduler
* 04: Listor
* 05: Dictionaries och tupler
* 06: Filer
* 07/10: Projekt och examinationer




## Ambitioner
<hr style="color:green">

- Kunna utveckla simpla program
- Lär dig verktyg, miljöer och felsökning




## Farhågor
<hr style="color:green">

- Nybörjare: Mycket nytt på en gång
- Känsla: Det är mycket nytt på en gång och det är nya saker varje vecka.
- Önskan: Jag vill ha mer tid att lära mig konstruktioner.
- Utmaning: Förstå konstruktionerna och använda rätt saker på rätt sätt vid rätt tillfälle.




## Labbmiljö
<hr style="color:green">

- Windows, Mac, Linux
- Text editor
- Terminal Bash
- Kursrepo GitHub
- Python
- UV
- Ruff





## Varför Python?

<hr style="color:green">

* Populäraste programmeringsspråket enligt internet.
* AI
* Näst bästa språket
* Lätt syntax
* Cross platform
* Stort community
* Excel





<h2 style="color:green">Slut</h2>

Börja jobba nu?

Installer labbmiljö, skapa kursrepot och sätt upp utvecklingsmiljö.
